...
**Justifications:** We chose Support Vector Machine to find the maximum margin hyperplane that optimally separates NFL wins from losses. SVM is particularly effective for binary classification problems like game outcome prediction.

Key advantages for our project:
- Finds the optimal decision boundary with maximum margin between classes
- Only support vectors (most important games) determine the final model
- Kernel methods allow capturing non-linear patterns in game data
- Robust to outliers and provides reliable probability estimates for betting
...
#### Test Accuracy
- Achieved **65.2%**, which exceeds our 55-60% target and the 52.4% break-even threshold
- Support Vectors: 894 out of 1,126 training games (79.4%)
- Indicates complex decision boundary requiring most training data as support vectors
...
- A perfect model = 0.0
- A naive model (50/50 every time) = 0.25  
- Our SVM = 0.223 → indicates good level of predictive accuracy
...
**4-Panel Analysis:**
- **ROI by Confidence:** Shows increasing ROI with higher confidence thresholds
- **Support Vector Ratio:** 79.4% of training data serves as support vectors
- **Accuracy by Confidence:** High confidence predictions achieve 77% accuracy
- **Kernel Comparison:** Linear kernel outperformed RBF kernel
...
**ROI Performance:**
- ROI increases dramatically with confidence level from 26.8% to 51.8%
- Risk vs Reward analysis shows optimal betting strategy at 70% confidence
- Higher confidence leads to fewer games but significantly more profitable outcomes
...
**Key Insights:**
- **Probability Distribution:** Well-calibrated predictions across confidence spectrum
- **Accuracy by Confidence:** Clear improvement from low (59%) to high (77%) confidence
- **Profit Analysis:** Consistent profit generation across confidence levels
- **Timeline Performance:** Steady profit growth to $3,160 over test period

#### Hyperparameter Optimization Results

**Best Parameters Found:**
- **Kernel:** Linear (surprisingly outperformed RBF)
- **C Parameter:** 0.1 (soft margin approach)
- **Gamma:** scale (default scaling for RBF attempts)
...
- **894 support vectors** out of 1,126 training games (79.4%)
- High support vector ratio indicates complex decision boundary
- Only these critical games define the entire classification model
- Demonstrates SVM's focus on boundary cases rather than all training data
